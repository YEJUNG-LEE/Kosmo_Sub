	...
	<script>
		...
		$(function() {
			$("#home_icon_img").click(function(){
				...
				location.href = "./../main_test1/main.jsp" ;
			});
			...
			$("#memberInfo").click(function(){
				document.getElementById('meInfo_form').submit();
			});
	    ...
	    <ul class="navigation-mainmenu">
	   	...
	   	<div class="navigation-right">
	        ...
	        <ul class="navigation-submenu">
				...
				<c:if test="${whologin != 0}">
					 <li id="goto_memberMyinfo">
					 	<c:if test="${ whologin == 2 }">
					 		<form action="<%=yesForm%>" method="post" id="">
								<input type="hidden" name="command" value="meList">
								<input type="submit" value="${myinfo}" style="background-color: black; border: 0px;">
							</form>
					 	</c:if>
					 	<c:if test="${ whologin == 1 }">
				           	<form action="<%=yesForm%>" method="post" id="">
								<input type="hidden" name="command" value="meMyinfo">
								<input type="submit" value="${myinfo}" style="background-color: black; border: 0px;">
							</form>
	            ...
	            <!-- 로그인 했을 경우 로그아웃, 로그아웃인 경우 로그인 -->
	            <c:if test="${whologin == 0}">
		            <li id="goto_memberLoginForm">로그인</li>
				</c:if>
				<c:if test="${whologin != 0}">
					<li id="goto_memberLogoutForm">
						<a href="<%=noForm%>${logoutCheck}">
							로그아웃
						</a>
					</li>
				</c:if>
	        ...
	        <div class="navigation-cart">
	        	<div onclick="location.href='<%=noForm%>maList'">
		        	<i></i>
	        	</div>
	           	<!---->
	           	</div> 
	           	<c:if test="${ whologin == 2 }">
		           	<div class="navigation-side" id="adminInfo">
		           	</div>
			 	</c:if>
	           	<c:if test="${ whologin == 1 }">
		           	<div class="navigation-side" id="memberInfo">
			           	<form action="<%=yesForm%>" method="post" id="meInfo_form">
							<input type="hidden" name="command" value="meInfo">
						</form>
		           	</div>
			 	</c:if>
	       	</div>
